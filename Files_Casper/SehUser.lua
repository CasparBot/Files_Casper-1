...
redis:del(bot_id..'Casper:Seh:User'..msg.chat_id_)  
send(msg.chat_id_, msg.id_,'☑┇تم تفعيل امر صيح') 
return false
end
if text == 'تعطيل صيح' and Owner(msg) then  
redis:set(bot_id..'Casper:Seh:User'..msg.chat_id_,true)  
send(msg.chat_id_, msg.id_,'☑┇تم تعطيل امر صيح') 
return false
end
if text and text:match("^صيح @(.*)$") then
local username = text:match("^صيح @(.*)$")
if not redis:get(bot_id..'Casper:Seh:User'..msg.chat_id_) then
