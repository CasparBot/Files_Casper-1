...
redis:set(bot_id..'Casper:SH:BOT'..msg.chat_id_,true)
return false
end
if text == 'تعطيل تنبيه الاشعار' and Owner(msg) then
send(msg.chat_id_, msg.id_,'📮┇ تم تعطيل التنبيه عل شعار  !')
redis:del(bot_id..'Casper:SH:BOT'..msg.chat_id_)
return false
end
if text and text:match('^وضع الشعار (.*)$') and Owner(msg) then
local SHGP = text:match('^وضع الشعار (.*)$')
redis:set(bot_id..'Casper:SH:BOT:GP'..msg.chat_id_,SHGP)
send(msg.chat_id_, msg.id_,'📮┇تم حفظ شعار المجموعه \n')
end
end
if text and redis:get(bot_id..'Casper:SH:BOT'..msg.chat_id_) and not Owner(msg) then
if tonumber(redis:get(bot_id..'Casper:SH:NUM'..msg.chat_id_..msg.sender_user_id_) or 0) > 3 then
KickGroup(msg.chat_id_,msg.sender_user_id_) 
send(msg.chat_id_, msg.id_,'📮┇ تم طردك من المجموعة لعدم احترام قوانين المجموعة')
redis:del(bot_id..'Casper:SH:NUM'..msg.chat_id_..msg.sender_user_id_)
else
tdcli_function ({ID = 'GetUser',user_id_ = msg.sender_user_id_},function(arg,data) 
local last_ = data.last_name_ or ''
local first_ = data.first_name_ or ''
local taha = (first_..''..last_)
local taha1 = (redis:get(bot_id..'Casper:SH:BOT:GP'..msg.chat_id_) or '')
if string.find(taha,taha1) == nil then
local taha = tonumber((redis:get(bot_id..'Casper:SH:NUM'..msg.chat_id_..msg.sender_user_id_) or 0))
if taha == 1 then
...
redis:incrby(bot_id..'Casper:SH:NUM'..msg.chat_id_..msg.sender_user_id_,1)
end
end,nil)  
return false
end
